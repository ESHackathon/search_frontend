var app = angular.module('search-strategy', []);
    app.controller('relevant', function($scope, $locale, $location, $http, $filter, $interval) {
      ...
      $scope.draftString = "(arctic AND Shrub*)";
      $scope.textareaText = $scope.draftString;
      ...
      var calculatSearchHits = function(search_strategy, callback){
        if(search_strategy){
      ...
      var extractKeywordsFromText = function(text){
        if(text){
          $http.post($scope.baseURL + ":5003/keyword-extraction", text).then(
            function(response){
              ...
              getUsedKeywords($scope.textareaText);
              // {"pubmed": 222072, "crossref": 1396698}%
            }
          );
        }
    ...
    // {
    //     var textAreaKeyWordsArray = $scope.textareaText.split(" ");
    //
    //     $scope.suggested=[];
    //     $scope.used=[];
    //
